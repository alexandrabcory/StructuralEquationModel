...
config_model <- function(dependent_var, latent_vars) {
  ...
  # Collect a list of latent variables
...
# Inputs: (1) the model, (2) the data, (3) the dependent var, (4) a list of latent vars 
# (this list can be anywhere from 1-4 items long)
save_observed_vs_modeled_plot <- function(sem_model, data, dependent_var, latent_vars) {
  
  # Generate predicted values
  sem_preds <- lavPredict(sem_model, type = "yhat")
  
  # Select the appropriate dependent variable for prediction
  sem_preds <- data.frame(sem_preds) %>% pull(dependent_var)
  ...
  # Save the plot
  filename <- paste(c(dependent_var, latent_vars), collapse = "_")
  ggsave(filename = paste0(filename, ".pdf"), plot = p, width = 12, height = 10, dpi = 150)
}

# Main function that integrates all the steps
# Inputs: (1) the path to the data csv file (2) the path to the csv file that defines which 
# latent variables correspond to each model (there are 15 models, each with varying combinations of latent variables)
run_analysis <- function(data_file_path, latent_var_models_file_path) {
  ...
  for (dependent_var in all_dependent_vars) {
    for (i in 1:nrow(latent_var_models)) {
      ...
      # run the model
      sem_model <- sem(working_model, data = sem_data)
      
      # make and save a plot of observed vs. measured values
      save_observed_vs_modeled_plot(sem_model, sem_data, dependent_var, latent_vars)
    }
  }
}
